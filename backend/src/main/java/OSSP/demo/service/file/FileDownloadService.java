...
public class FileDownloadService {
    ...
    @Transactional
    public ResponseEntity downloadFileVersion(String studentId, Long fileId, Long fileVersionId, Long teamId) {
    ...
    @Transactional
    public ResponseEntity downloadFile(String studentId, Long fileId, Long teamId) {
        //findby조건을 달때, 해당 레포지토리의 pk부터 달아야함.
        try {
            User user = userRepository.findByStudentId(studentId).get();
            Member member = memberRepository.findByUserIdAndTeamId(user.getId(), teamId).orElse(null);
...
//            }
            ...
            FileVersionDto fileVersionDto = FileVersionDto.builder().
                    s3Url(findFile.getS3FileUrl()).
                    fileName(findFile.getRealFileName()).build();
